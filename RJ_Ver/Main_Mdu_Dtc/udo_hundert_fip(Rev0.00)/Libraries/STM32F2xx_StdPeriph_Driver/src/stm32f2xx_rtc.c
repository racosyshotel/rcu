...
//void Write_BKPSRAM(uint32_t BKP_SRAM_ADDR, uint32_t Data)
void Write_BKPSRAM(uint32_t BKP_SRAM_ADDR, uint8_t Data)
{
  __IO uint32_t tmp = 0;
  
  /* Check the parameters */
  assert_param(IS_BKP_SRAM_ADDR(BKP_SRAM_ADDR));

  tmp = BKPSRAM_BASE + BKP_SRAM_ADDR;
//  tmp += (BKP_SRAM_ADDR * 4);
  /* Write the specified register */
//  *(__IO uint32_t *)tmp = (uint32_t)Data;
  *(__IO uint32_t *)tmp = (uint8_t)Data;
}

//uint32_t Read_BKPSRAM(uint32_t BKP_SRAM_ADDR)
uint8_t Read_BKPSRAM(uint32_t BKP_SRAM_ADDR)
{
  __IO uint32_t tmp = 0;
