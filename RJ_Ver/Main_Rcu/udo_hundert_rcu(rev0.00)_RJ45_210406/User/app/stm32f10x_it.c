  ...
  if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET){     // rx interrupt check
    ...
    if(!f_uart1_com_start){
      ...
      if((uart1_sbuf == ')') && (uart1_rxding_point+1 == uart1_rxding_buf[1])){
        ...
        if(uart1_rxding_point > 25) f_uart1_com_start = 0;
...
void USART3_IRQHandler(void){
  ...
  if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET){     // rx interrupt check
    ...
    {
      if(uart3_sbuf == 0x01 || uart3_sbuf == 0x02 || uart3_sbuf == 0x03)        //처음에 에어컨의 주소값을 총 3대까지 인정해준것
      {
        f_uart3_com_start = 1;
        uart3_rxding_point = 0;                                                 //데이터 길이를 0으로 클리어 시켜줌
        uart3_rxding_buf[uart3_rxding_point++] = uart3_sbuf;                    //처음에 에어컨의 주소값을 받은 부분
      }
    }    
    else                //처음 주소값을 잘 받은뒤
    {                                                     
