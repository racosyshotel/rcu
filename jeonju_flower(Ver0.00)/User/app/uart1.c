...
void cb_data_send(unsigned char send_cmd)
{
  ...
  switch(send_cmd){
    case 'S':   //Standard data send
      ...
      uart1_tx_data_buf[i++] = room_data_buf[67];                   //Sub device (k/s, c/i, nt{2} 통신상태 및 설치)
...
void main_rxd_check(void)
{
  ...
  switch(uart1_rxd_buf[3]){
    case 'G':   //Basic data & Group o
      ...
      if(uart1_rxd_buf[6] & 0x01) f_bath_control = 0;                 //화장실 전등 OFF
      else f_bath_control = 1;                                               //화장실 전등 ON
      
      if(uart1_rxd_buf[6] & 0x02) led_bright_control = 1;            //통신LED 정상 밝기
      else led_bright_control = 0;                                          //통신LED  흐림 밝기
      
      if((uart1_rxd_buf[6] & 0x04) != rcu_dip_aircon)
      {
          rcu_dip_aircon = (uart1_rxd_buf[6] & 0x04);
          UART3_initial();
      }
      
      //ts_mode_set = (uart1_rxd_buf[6] & 0x01); // 객실 바닥제어 가능/불가능 설정값 (FIP -> ACU)
      break;
