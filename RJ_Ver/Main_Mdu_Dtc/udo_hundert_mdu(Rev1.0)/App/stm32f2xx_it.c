...
unsigned char uart1_temp_buf;

  if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET){ 	 // rx interrupt check
    uart1_temp_buf = (u16)(USART1->DR & (u16)0x01FF);
	}
}
  ...
  unsigned char uart2_sbuf;
    ...
    if(!f_uart2_com_start){
      ...
      if(uart2_sbuf == ')'){
        ...
        if(uart2_rxding_point > 20) f_uart2_com_start = 0;
...
//      if(uart3_rxding_point == 1) uart3_rx_length = uart3_sbuf;
      if(uart3_rxding_point == 1){
        uart3_rx_length = uart3_sbuf;
        if(uart3_rx_length > 99) f_uart3_com_start = 0;
      }
      //if(uart3_sbuf == '}'){
