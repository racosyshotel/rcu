...
u8 ix;
    ...
    if(f_dnd_sign)
    {
        if(++dnd_count > 150)//if(++dnd_count>150)                //2018 09 21 cg modify
        {
            dnd_count = 0;
            f_dnd_ok = 1;
        }
    ...
    /*
      if(!f_uart3_data_send_3)
       {
         if(++uart3_send_time_3 > 10000) //삼성PLC 상태주기 10초
         {
           uart3_send_time_3 = 0;
           f_uart3_data_send_3 = 1;
         }
       }
     
       if(!f_uart3_data_send_4)
       {
         if(++uart3_send_time_4 > 30000) //삼성PLC 센서 설정 주기 30초
         {
           uart3_send_time_4 = 0;
           f_uart3_data_send_4 = 1;
         }
       } 
       if(++uart3_send_time_2 > 40000) //삼성PLC Address 요청 주기 40초
       {
         uart3_send_time_2 = 0;
     ...
     // PLC AIRCON RX #1//
      if(f_plc_add_toggle[0]) //수신 안됨 FLAG SET (AIRCON)
      {
        if(++plc_reset_air_timer[0] > 20000) //20sec state
        {
            plc_reset_air_timer[0] = 0;
            
            aircon_no_respones[0] = 1; 
        }
      }
      else plc_reset_air_timer[0] = 0;    
*/
     // PLC AIRCON RX  #2//
      /* if(f_plc_add_toggle[1]) //수신 안됨 FLAG SET (AIRCON)
      {
        if(++plc_reset_air_timer[1] > 20000) //20sec state
        {
            plc_reset_air_timer[1] = 0;
            
            aircon_no_respones[1] = 1; 
        }
      }
      else plc_reset_air_timer[1] = 0;         
     */
    ...
    for(int i=0; i<2; i++)
    {
      ...
      for(int i=0; i<4; i++)     //온도센서
     {
        if(ts_device_falling_count[i] == 20)    //통신 이상시
        {
           if(i==0)room_data_buf[24] &= 0x7f;
           if(i==1)room_data_buf[25] &= 0x7f;
           if(i==2)room_data_buf[26] &= 0x7f;
           if(i==3)room_data_buf[27] &= 0x7f;
        }
        else
        {
    ...
    ////DTC LG/SAMSUNG AIRCON 21.01.11////
    if(!rcu_dip_aircon)     //SAMSUNG?
    {
      if(!f_uart3_data_send_3_ss)
      {
        if(++uart3_send_time_3_ss > 10000) //삼성 PLC 상태주기 10초
        {
          uart3_send_time_3_ss = 0;
          f_uart3_data_send_3_ss = 1;
        }
      }
      
      if(!f_uart3_data_send_4_ss)
      {
        if(++uart3_send_time_4_ss > 30000) //삼성 PLC 센서 설정 주기 30초
        {
          uart3_send_time_4_ss = 0;
          f_uart3_data_send_4_ss = 1;
        }
      }
      
      if(++uart3_send_time_2_ss > 40000) //삼성 PLC Address 요청 주기 40초
      {
        uart3_send_time_2_ss = 0;
        f_uart3_data_send_2_ss = 1;
      }
      
       if(++f_uart3_data_send_time_ss > 500) //삼성 PLC 연동 완료시 작동 타이머
       {
          f_uart3_data_send_time_ss = 0;
          f_uart3_data_send_ok = 1;
       }         
      ...
      // PLC AIRCON RX #1//
         ...
         // PLC AIRCON RX  #3//
          if(f_plc_add_toggle[2]) //수신 안됨 FLAG SET (AIRCON)
          {
            if(++plc_reset_air_timer[2] > 20000) //20sec state
            {
                plc_reset_air_timer[2] = 0;
                
                aircon_no_respones[2] = 1; 
            }
          }
          else plc_reset_air_timer[2] = 0;
    }
    else //LG?
    {
      
        if(++uart3_send_time_2 > 1000) //1초마다
        {
          uart3_send_time_2 = 0;
...
void dip_switch_read(void)
{
  ...
  //main_id_buf = dip_switch_buf[0] & 0x3f; //객실 어드레스 할당 (주의) 본물량?? 목업룸??
  ...
  if(!main_comm_rx_ok){ //메인통신 하지않을떄 ??
    ...
    if(dip_switch_buf[1] & 0x02){
      room_data_buf[74] = 2;          //환절기 적용
       ...
       if(dip_switch_buf[1] & 0x01){     // Buffer : 1 = dip 아래방향
    ...
    if(dip_switch_buf[1] & 0x08) rcu_dip_aircon = 0;
    else rcu_dip_aircon = 1;
    
    if(rcu_dip_aircon_backup != rcu_dip_aircon)
    {
       rcu_dip_aircon_backup = rcu_dip_aircon;
       UART3_initial();
    }
  }
  ...
  /*if(dip_switch_buf[1] & 0x08)         //화장실 전열 추가
  {    
    room_data_buf[60] |= 0x02;        //동시제어
    room_data_buf[61] |= 0x02;        //동시제어
    room_data_buf[62] |= 0x02;        //동시제어
  ...
  */
  ...
  switch(dip_switch_buf[1] & 0xc0){   //CB mode를 지정한다.
    ...
    case 0xc0:                                                                                                                                                                                                                  //////
            ...
            cb_mode = 0; //RCB-1
               ...
               room_data_buf[64] = 0x03;                                 // 라이트스위치 총수량 (16ea)
               room_data_buf[67] |= 0xf0;                                 // KS, CI, NT 총수량
    ...
    case 0x40: 
            ...
            cb_mode = 1; //RCB-2,3
               ...
               room_data_buf[16] |= 0x08;  //1번 온도센서 사용(실내)
               room_data_buf[16] |= 0x40;  //1번 온도센서 사용(바닥)
               room_data_buf[17] |= 0x08; // 2번 온도센서 사용
               room_data_buf[17] |= 0x40; // 2번 온도센서 사용
  ...
  if(dip_switch_buf[1] & 0x10) f_temp_test_mode = 0; //에어컨 에러 활성화 모드
  else f_temp_test_mode = 1; //에어컨 에러 비활성화 모드
  
  if(dip_switch_buf[1] & 0x20) f_send_tempsensor_data = 0; //에어컨 현재온도 비활성화
  else f_send_tempsensor_data = 1; //에어컨 현재온도 활성화 모드  
}
//Dip switch 1 
...
//----------- Dimmer Level reload ---------------------
  ...
  room_data_buf[93] = 0xff;   //Hour initial
  room_data_buf[94] = 0xff;
